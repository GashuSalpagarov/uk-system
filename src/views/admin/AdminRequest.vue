<template>
  <v-container class="request-detail-bg pa-0" fluid>
    <div class="d-flex mb-6">
      ...
      <h2 class="text-h4 font-weight-bold mb-0">Задача {{ request.id }}</h2>
      <v-chip
...
function goBack() {
  window.history.length > 1 ? window.history.back() : router.push('/admin/home')
}
const request = {
  id: 1,
  status: 'open',
  date: '2024-07-20',
  address: 'ул. Кленовая, 123',
  category: 'Сантехнический ремонт',
  urgency: 'urgent',
  description: 'Плановые работы по замене труб в подвале. Необходим доступ к техническим помещениям.',
  photos: [
